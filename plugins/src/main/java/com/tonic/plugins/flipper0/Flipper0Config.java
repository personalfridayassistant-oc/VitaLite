...
public interface Flipper0Config extends Config
{
    ...
    @ConfigItem(keyName = "horizonMinutes", name = "Backtest horizon (minutes)", description = "Horizon used when querying backtests endpoint", position = 8)
    default int horizonMinutes() { return 30; }

    @ConfigItem(keyName = "lookbackHours", name = "Backtest lookback (hours)", description = "Lookback window used when querying backtests endpoint", position = 9)
    default int lookbackHours() { return 24; }

    @ConfigItem(keyName = "maxDataAgeSeconds", name = "Max suggestion age (seconds)", description = "Ignore stale suggestions", position = 10)
    default int maxDataAgeSeconds() { return 240; }

    @ConfigItem(keyName = "staleOfferSeconds", name = "Cancel stale offers (seconds)", description = "Cancel offers older than this", position = 11)
    default int staleOfferSeconds() { return 90; }

    @ConfigItem(keyName = "loopDelayTicks", name = "Loop delay (ticks)", description = "Delay between trading cycles", position = 12)
    default int loopDelayTicks() { return 1; }


    @ConfigItem(keyName = "cycleAfterAttempts", name = "Cycle after failed attempts", description = "Move to next item after this many buy failures/stale cancellations", position = 13)
    default int cycleAfterAttempts() { return 3; }

    @ConfigItem(keyName = "apiBaseUrl", name = "VitaLite API base URL", description = "Example: http://192.168.1.27:3015/api/v1", position = 14)
    default String apiBaseUrl() { return "http://192.168.1.27:3015/api/v1"; }

    @ConfigItem(keyName = "f2pOnly", name = "F2P-only suggestions", description = "Only fetch and trade free-to-play (non-members) items", position = 15)
    default boolean f2pOnly() { return false; }

    @ConfigItem(keyName = "showOverlay", name = "Show overlay", description = "Show Flipper0 in-game overlay", position = 16)
    default boolean showOverlay() { return true; }

    @ConfigItem(keyName = "overlayOffsetY", name = "Overlay offset Y", description = "Vertical offset for Flipper0 overlay", position = 17)
    default int overlayOffsetY() { return 40; }

    @ConfigItem(keyName = "blacklistItemIds", name = "Blacklist item IDs", description = "CSV item IDs to always skip", position = 18)
