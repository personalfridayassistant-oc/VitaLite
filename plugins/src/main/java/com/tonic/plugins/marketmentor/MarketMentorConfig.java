...
public interface MarketMentorConfig extends Config
{
    ...
    default int maxGpPerTrade() { return 2_000_000; }
    ...
    default double minMarginPct() { return 1.2; }

    @ConfigItem(keyName = "minNetRoiPct", name = "Min net ROI %", description = "Minimum expected ROI after estimated GE tax", position = 8)
    default double minNetRoiPct() { return 0.6; }

    @ConfigItem(keyName = "minFiveMinuteVolume", name = "Min 5m volume", description = "Minimum 5-minute low/high volume for liquidity", position = 9)
    default int minFiveMinuteVolume() { return 25; }

    @ConfigItem(keyName = "maxDataAgeSeconds", name = "Max quote age (seconds)", description = "Skip items with stale latest quote timestamps", position = 10)
    default int maxDataAgeSeconds() { return 1800; }

    @ConfigItem(keyName = "buyBumpPct", name = "Buy bump %", description = "Buy above low price for faster fills", position = 11)
    default double buyBumpPct() { return 0.6; }

    @ConfigItem(keyName = "sellUndercutPct", name = "Sell undercut %", description = "Sell below high price for quicker exits", position = 12)
    default double sellUndercutPct() { return 0.7; }

    @ConfigItem(keyName = "panicDiscountPct", name = "Panic discount %", description = "Discount below low used when a holding times out", position = 13)
    default double panicDiscountPct() { return 1.0; }

    @ConfigItem(keyName = "staleOfferSeconds", name = "Stale offer cancel (seconds)", description = "Abort active offers older than this timeout", position = 14)
    default int staleOfferSeconds() { return 90; }

    @ConfigItem(keyName = "holdingTimeoutSeconds", name = "Holding timeout (seconds)", description = "Switch to panic sell after this duration", position = 15)
    default int holdingTimeoutSeconds() { return 210; }

    @ConfigItem(keyName = "loopDelayTicks", name = "Loop delay (ticks)", description = "Ticks between automation cycles", position = 16)
    default int loopDelayTicks() { return 2; }

    @ConfigItem(keyName = "autoOpenGe", name = "Auto-open GE", description = "Open GE by interacting nearby clerk when closed", position = 17)
