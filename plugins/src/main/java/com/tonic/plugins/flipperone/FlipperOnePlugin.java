...
public class FlipperOnePlugin extends VitaPlugin
{
    private static final Logger log = LoggerFactory.getLogger(FlipperOnePlugin.class);
    ...
    @Inject private FlipperOneConfig config;
    @Inject private Client client;
    @Inject private FlipperOnePanel panel;
    @Inject private FlipperOneOverlay overlay;
    ...
    {
        log.info("FlipperOne starting up");
        ...
        navButton = NavigationButton.builder().tooltip("FlipperOne").icon(icon).panel(panel).build();
    ...
    {
        log.info("FlipperOne shutting down");
    ...
    public void loop() throws Exception
    {
        ...
        if (currentSuggestion == null)
        {
            ...
            log.info("No active recommendation. status='{}' parsed={} accepted={} invalidPrice={} lowVolume={} stale={} membersBlocked={} untradeable={} lowRoi={} overBudget={} blacklisted={}",
                    statusText,
                    suggestions.size(),
                    stats.accepted,
                    stats.invalidPrice,
    ...
    private List<String> suggestionEndpoints()
    {
        ...
        endpoints.add(base + "/suggestions?limit=220&membersOnly=" + (WorldsAPI.inMembersWorld() && !config.f2pOnly()));
