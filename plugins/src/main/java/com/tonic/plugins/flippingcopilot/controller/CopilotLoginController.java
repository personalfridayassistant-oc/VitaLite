...
import com.tonic.plugins.flippingcopilot.ui.LoginPanel;
import com.tonic.plugins.flippingcopilot.ui.MainPanel;
...
public class CopilotLoginController {
    ...
    public CopilotLoginController(ApiRequestHandler apiRequestHandler,
        ...
        // Skip loading accounts and syncing flips if using mock login (offline mode)
        if (!copilotLoginRS.get().isMockLogin()) {
            loadCopilotAccounts(0);
        } else {
            log.info("Running in offline mode - skipping account loading and flip syncing");
        }

        copilotLoginRS.registerListener((s) -> {
            // When login response becomes null (logged out), reset everything
            if(s.loginResponse == null || s.isMockLogin()) {
                flipManager.reset();
    ...
    private void loadCopilotAccounts(int previousFailures) {
        ...
        Consumer<String> onFailure = (errorMessage) -> {
            // Only retry if not using mock login
            if (copilotLoginRS.get().isLoggedIn() && !copilotLoginRS.get().isMockLogin()) {
                ...
                log.info("skipping retry for account loading in offline/mock mode");
            }
        };
    ...
    private void syncFlips(int userId, Map<Integer, Integer> accountIdTime, int previousFailures) {
        ...
        Consumer<String> onFailure = (errorMessage) -> {
            ...
            if (!copilotLoginRS.get().isMockLogin()) {
                ...
                log.info("skipping retry for flip syncing in offline/mock mode");
            }
        };
    ...
    public void onLoginFailure(String errorMessage) {
        ...
        // Re-initialize with mock login state
        copilotLoginRS.update((s) -> {
            s.loginResponse = new LoginResponse("mock-jwt-token-for-offline-mode", 1, null);
            return s;
        });
