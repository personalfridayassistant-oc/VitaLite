...
import com.tonic.plugins.flippingcopilot.ui.*;
...
public class SuggestionController {
    ...
    public void getSuggestionAsync() {
        ...
        // If using local OSRS Wiki prices, skip login check and use local API
        if (config.useLocalOsrsWikiPrices()) {
        ...
        if (!copilotLoginRS.get().isLoggedIn() || copilotLoginRS.get().isMockLogin() || !osrsLoginManager.isValidLoginState()) {
            return;
        }
        if (suggestionManager.isSuggestionRequestInProgress()) {
            return;
